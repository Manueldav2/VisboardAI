...
async def enhance_notes(transcript: str, notes: str = "", title: str = "") -> str:
    ...
    sys = (
        ...
        "STRUCTURE — organize like an elite human note-taker (Granola-style), NOT a "
        "rigid template:\n"
        "- Open with a one-line **TL;DR** (2-3 sentences) capturing what the meeting "
        "was actually about and what was decided.\n"
        "- Then break the notes into 2-6 SECTIONS BY TOPIC. Each section gets a '## ' "
        "header named after the real thing discussed (e.g. '## Product Launch', "
        "'## Pricing & Monetization', '## Configure Demo'). Do NOT use generic headers "
        "like 'Key points' or 'Notes'.\n"
        "- Under each header, write tight, scannable bullets. Use indented sub-bullets "
        "(two spaces) for supporting detail. **Bold** the genuinely important bits — "
        "decisions, metrics, names, dollar amounts, commitments.\n"
        "- End with a '## Next steps' section: concrete action items as bullets, each "
        "in the form '- **Owner — task**' when an owner was named. Omit the section if "
        "nothing actionable came up.\n"
        "- If real questions were left open, you may add a short '## Open questions'.\n\n"
        "Be dense and specific — real names/terms/numbers from the transcript, no "
        "filler, no repetition, no throat-clearing. Weave in what the user jotted (it "
        "signals what matters to them). No preamble, no 'here are your notes'."
    )
    user = (
        (f"# {title}\n\n" if title.strip() else "")
